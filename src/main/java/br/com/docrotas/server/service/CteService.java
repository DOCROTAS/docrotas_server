...
import br.com.docrotas.server.entity.SituacaoDocumento;
import br.com.docrotas.server.entity.StatusProcessamento;
...
import br.com.docrotas.server.service.cte.AssinadorXMLCTe;
import br.com.docrotas.server.service.cte.GeradorXmlCte;
import br.com.docrotas.server.service.cte.RecepcaoCTeService;
import br.com.docrotas.server.service.cte.RetornoRecepcaoCTeService;
...
public class CteService {
	...
	private static final String PATH_CERTIFICADO = "C:\\Users\\lauro\\certificado.pfx";
	private static final String SENHA_CERTIFICADO = "Tk18@Kothe";
	private static final String PATH_CACERTS = "C:\\Users\\lauro\\carcets";
	private static final String CODIGO_UF = "17";
	private static final String VERSAO = "3.00";

	private static final String PATH_SCHEMA_ENVI_CTE = "C:\\Users\\lauro\\PL_CTe_300_NT2018.002\\enviCTe_v3.00.xsd";
	private static final String PATH_SCHEMA_CONS_RECI_CTE = "C:\\Users\\lauro\\PL_CTe_300_NT2018.002\\consReciCTe_v3.00.xsd";
...
//	private static final String URL_CTE_RET_RECEPCAO = "https://hcte.fazenda.mg.gov.br/cte/services/CteRetRecepcao";
	...
	private static final String URL_CTE_RECEPCAO = "https://cte-homologacao.svrs.rs.gov.br/ws/cterecepcao/CteRecepcao.asmx";
	private static final String URL_CTE_RET_RECEPCAO = "https://cte-homologacao.svrs.rs.gov.br/ws/cteretrecepcao/cteRetRecepcao.asmx";
	

	@Autowired
	private CteRepository cteRepository;
	
	public Cte buscarPorId(Long id) {
		return cteRepository.findOne(id);
	}
	
	public Cte salvar(Cte cte) {
		cte.setTomador(cte.getRemetente());
		
		return cteRepository.save(cte);
	}
	
	public void excluir(Cte cte) {
		cteRepository.delete(cte);
	}
	...
	public void consultarRetornoLote(Cte cte) throws Exception {
		...
		if (SituacaoDocumento.AGUARDANDO_AUTORIZACAO.equals(cte.getSituacao())) {
			if (StringUtils.isNotEmpty(cte.getProtocoloLote())) {
				...
				String xml = xmlOutputter.outputString(geradorXmlCte.getConsReciCTe(cte.getProtocoloLote(), TipoAmbienteEmissao.HOMOLOGACAO));
				...
				if (StatusProcessamento.AUTORIZADO.equals(respostaRetornoRecepcao.getStatusProcessamentoCTe())) {
					...
					if (respostaRetornoRecepcao.getDtRecebimento() != null) {
						cte.setDtProcolocoloAutorizacao(respostaRetornoRecepcao.getDtRecebimento());
					}
	...
	private RespostaRecepcao processarRespostaRecepcao(String xml) throws JDOMException, IOException {
		...
		resposta.setTipoAmbienteEmissao(TipoAmbienteEmissao.HOMOLOGACAO);
	...
	public void enviarEmail(Cte cte) throws EmailException {
		SimpleEmail mail = new SimpleEmail();
		mail.setFrom("laurocoelhoc@gmail.com", cte.getEmpresa().getRazao());
		mail.setSubject("DOCROTAS CT-e nº" + cte.getNumero());
		
		StringBuilder stb = new StringBuilder();
		stb.append(cte.getRemetente().getNome());
		stb.append("\n\n");
		stb.append("A Transportadora ");
		stb.append(cte.getEmpresa().getRazao());
		stb.append(" emitiu um CT-e para você.");
		stb.append("\n\n");
		stb.append("Chave de acesso: ");
		stb.append(cte.getChaveAcesso());
		stb.append("\n\n");
		stb.append("Consulte a validade através: ");
		stb.append("https://hom.cte.fazenda.gov.br/portal/consulta.aspx?tipoConsulta=completa&tipoConteudo=mCK/KoCqru0=");
		
		mail.setMsg(stb.toString());
		mail.setSSLOnConnect(true);
		mail.setAuthentication("laurocoelhoc@gmail.com", "scania112hw");
		mail.setHostName("smtp.gmail.com");
		mail.setSmtpPort(465);
		mail.addTo("laurocoelhoc@gmail.com", "DOCROTAS");
		mail.send();
	}
}
